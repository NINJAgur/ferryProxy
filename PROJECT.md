...
- [x] **17.9** **A refund revokes access, and needs no code.** Tested with a real
      ...
      Verified on live data — the refunded transaction disappeared, the two sandbox
...
- [x] **17.10** `ALLOW_SANDBOX_PURCHASES` is **off**, which is what production needs:
      ...
      tester cannot exercise a purchase without paying for it — turn it on
      deliberately and briefly if that is ever wanted, never as a resting state
