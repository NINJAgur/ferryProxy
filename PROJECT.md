...
- [x] **17.7** **The pool is keyed by the store's transaction id, not the install.**
      The app-user id is regenerated by a reinstall, so a restored purchase looked up
      a row that did not exist and was handed a fresh 500. It now resolves to the
      oldest real transaction behind the entitlement, and the allowance is 500 x the
...
- [x] **17.2** **Render moved to Starter with a 1GB disk at `/var/data`.** The free
      ...
      into the volume, verified by reading both files back off it
